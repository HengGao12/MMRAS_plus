...
class MaskFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # load text embeddings, added by GH
        self.text_embeddings = torch.load(text_embeddings_path).cuda()
        self.text_regularization = text_regularization
        
        if self.text_regularization:
            self.projection = nn.Conv2d(1024, 19, kernel_size=1).cuda()
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "text_regularization": cfg.MODEL.TEXT_REGULARIZATION,  # GH added
            # inference
    ...
    def forward(self, batched_inputs):
        ...
        # GH added
        if self.text_regularization:
            # feat_temp = features['res4'] / features['res4'].norm(dim=1, keepdim=True)
            temp = F.conv2d(features['res4'], self.text_embeddings[:,:,None,None]) # text features
            proj_feat = self.projection(features['res4'])
            temp = F.softmax(temp, dim=1)
            # proj_feat = F.softmax(proj_feat, dim=1)
            # loss_mse = nn.MSELoss()(proj_feat, temp).to(features['res4'].device) # baseline
            loss_tr = nn.CrossEntropyLoss()(proj_feat / 4.0, temp).to(features['res4'].device) # newly added 10-15
            # ipdb.set_trace()
        ...
        '''
        # outputs['pred_logits'] = outputs['pred_logits'] / 5.0  # 5.0 is the best temperature
